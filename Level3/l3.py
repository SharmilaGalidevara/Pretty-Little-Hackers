# =====================================================================
#  PARKMIND v3.2  —  LEVEL 2 API-DRIVEN 7-ZONE SAFE Edition
...
# A barrier name may legitimately appear in more than one topology role in the
# simulator payload (for example, the supplied Level-2 map reports gate7 both
# as a perimeter barrier and as a ZONE4 barrier).  Keep every membership instead
# of letting the gates{} name-keyed cache erase that relationship.
gate_memberships = defaultdict(set)   # gate name -> {"PERIMETER", "ZONE4", ...}
shared_topology_gates = set()         # gate names with more than one membership
...
def resolve_gate_names(barrier_data):
    ...
    The simulator can expose the same barrier name in more than one topology
    role.  The current 7-zone map, for example, reports ``gate7`` once with an
    empty zoneParent and once with ``ZONE4``.  A plain name-keyed dictionary
    would lose that relationship, so topology is stored separately as a set of
    memberships for each gate.

    Classification remains fully API-driven:
      - perimeter_gates: any record whose zoneParent is empty
      - zone_gates[ZONE]: any record returned inside that zone
      - shared_topology_gates: names appearing in two or more roles

    ENTRY_GATE/EXIT_GATE are compatibility aliases only.  Entry prefers a
    healthy perimeter gate that is already Open/Opening (the simulator normally
    exposes the spawn barrier this way).  Exit prefers a different perimeter-
    only barrier, avoiding a shared zone/perimeter gate when possible.
    """
    global ENTRY_GATE, EXIT_GATE, perimeter_gates, zone_gates
    global gate_memberships, shared_topology_gates
    ...
    for g in barrier_data:
        ...
        if not name:
            continue
        records_by_name[name].append(dict(g))
        zone = str(g.get("zoneParent") or "").strip()
        if zone:
            if name not in by_zone[zone]:
                by_zone[zone].append(name)
            memberships[name].add(zone)
        else:
            if name not in perimeter:
                perimeter.append(name)
            memberships[name].add("PERIMETER")

    perimeter_gates = list(perimeter)
    zone_gates = defaultdict(list, {z: list(v) for z, v in by_zone.items()})
    gate_memberships = defaultdict(set, {n: set(v) for n, v in memberships.items()})
    shared_topology_gates = {n for n, roles in gate_memberships.items() if len(roles) > 1}

    # Compatibility aliases are still API-derived.  Prefer an already-open,
    # healthy perimeter gate for entry; this matches the spawn-route behaviour
    # without hardcoding a gate number.
    candidate_pool = perimeter if perimeter else list(dict.fromkeys(names))

    def _record_is_healthy(rec):
        return not bool(rec.get("broken")) and not bool(rec.get("isUnderMaintenance"))

    open_healthy = []
    healthy = []
    for name in candidate_pool:
        recs = records_by_name.get(name, [])
        if recs and all(_record_is_healthy(r) for r in recs):
            healthy.append(name)
            if any(str(r.get("state") or "") in ("Open", "Opening") for r in recs):
                open_healthy.append(name)

    ENTRY_GATE = (open_healthy or healthy or candidate_pool or [None])[0]

    remaining = [n for n in candidate_pool if n != ENTRY_GATE]
    # Prefer a perimeter-only gate for the exit alias. Shared gates such as the
    # supplied gate7 remain usable on their zone route but are not chosen as the
    # generic exit alias unless there is no cleaner alternative.
    perimeter_only = [
        n for n in remaining
        if gate_memberships.get(n, set()) == {"PERIMETER"}
    ]
    exit_pool = perimeter_only or remaining
    if not exit_pool:
        exit_pool = [n for n in dict.fromkeys(names) if n != ENTRY_GATE]
    EXIT_GATE = exit_pool[-1] if exit_pool else ENTRY_GATE

    duplicate_names = sorted(
        [n for n, recs in records_by_name.items() if len(recs) > 1],
        key=lambda x: (natural_spot_key(x), x),
    )
    log_decision(
        "", "GATE_TOPOLOGY",
        f"perimeter={perimeter_gates}; zone_gates={dict(zone_gates)}; "
        f"shared={sorted(shared_topology_gates)}; duplicates={duplicate_names}; "
        f"aliases=({ENTRY_GATE},{EXIT_GATE})",
        "All barrier identifiers and zone relationships came from /list-barriers; shared names are preserved safely."
    )
...
def sync_state():
    ...
    with state_lock:
        ...
        for raw in barrier_data:
            if raw.get("name"):
                grouped_barriers[str(raw["name"])].append(dict(raw))

        for name, records in grouped_barriers.items():
            # Merge duplicate-name records conservatively.  The command API is
            # name-based, so one cache row represents the physical identifier,
            # while topology memberships are retained separately below.
            merged = dict(records[-1])
            zone_parents = []
            is_perimeter = False
            for rec in records:
                zone = str(rec.get("zoneParent") or "").strip()
                if zone:
                    if zone not in zone_parents:
                        zone_parents.append(zone)
                else:
                    is_perimeter = True
            merged["zoneParents"] = zone_parents
            merged["isPerimeter"] = is_perimeter
            merged["broken"] = any(bool(r.get("broken")) for r in records)
            merged["isUnderMaintenance"] = any(bool(r.get("isUnderMaintenance")) for r in records)
            merged["usage_count"] = prev_gates.get(name, {}).get("usage_count", 0)
            gates[name] = merged
...
def gate_required_by_live_movement(name):
    """Return True when an automatic close could break a live vehicle route.

    This is especially important for shared-topology identifiers.  In the
    supplied 7-zone level ``gate7`` is both a perimeter member and a ZONE4
    member; perimeter cleanup must therefore not close it while ZONE4 is still
    being crossed.  The same guard also protects ordinary entry/exit barriers.
    """
    name = str(name or "")
    if not name:
        return False
    ...
    with state_lock:
        if entry_active and name in set(entry_active.get("required_gates") or []):
            return True
        if exit_active and name in set(exit_active.get("required_gates") or []):
            return True

        active_zones = set()
        for zone_name, owner in zone_entry_active.items():
            if owner:
                active_zones.add(str(zone_name or ""))
        for _plate, zone_name in exit_route_zone.items():
            if zone_name:
                active_zones.add(str(zone_name))

        for zone_name in active_zones:
            if name in set(zone_gates.get(zone_name, [])):
                return True
...
def close_gate(name, force=False, manual=False):
    ...
    # Never let an automatic cleanup close a barrier that is still part of an
    # active entry/exit or zone crossing. Manual operator action remains explicit.
    if not manual and gate_required_by_live_movement(name):
        log_decision(
            "", "GATE_CLOSE_DEFERRED", name,
            f"Barrier still belongs to a live route; memberships={sorted(gate_memberships.get(name, set()))}."
        )
        return False
...
def complete_zone_entry_crossing(plate, zone_name):
    ...
    # A waiting entry can now reconsider the next available API-discovered zone immediately.
...
def choose_spot(car_type, planned_minutes, avoid_busy_zone_gates=False):
    ...
    LEVEL-2 ADMISSION POLICY:
      ...
      * With seven zones, an initially empty site naturally spills across
        ZONE1 -> ZONE2 -> ... -> ZONE7 before returning to the least-loaded zone.
        The same rule works for any number of API-discovered zones.
...
def process_entry_queue():
    ...
    with state_lock:
        ...
        if demo_state.get("live_full_next_arrival"):
            ...
            # Admission-aware selection: if one zone gate path is busy,
            # immediately try the next least-loaded API-discovered zone.  This
            # works unchanged for the supplied ZONE1..ZONE7 layout.
...
if __name__ == "__main__":
    ...
    print("\n============================================")
    print("  PARKMIND v3.2 — Level-2 7-Zone Compliance + Preserved Parking Workflow")
