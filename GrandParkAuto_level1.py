...
def refresh_alert_from_car(alert_id, plate):
    ...
    stay_minutes = int(car.get("total_stay_minutes") or 0)
    if not stay_minutes and entry_time and event_time:
        try:
            entered = datetime.strptime(entry_time, "%Y-%m-%d %H:%M:%S")
            event_dt = datetime.strptime(str(event_time)[:19], "%Y-%m-%d %H:%M:%S")
            stay_minutes = max(1, math.ceil((event_dt - entered).total_seconds() / 60))
        except Exception:
            stay_minutes = int(car.get("billable_minutes") or 0)
...
def record_alert(
    ...
    # If the trip is still open, calculate duration up to the alert time.
    if not stay_minutes and entry_time:
        try:
            entered = datetime.strptime(entry_time, "%Y-%m-%d %H:%M:%S")
            alerted = datetime.strptime(event_time, "%Y-%m-%d %H:%M:%S")
            stay_minutes = max(1, math.ceil((alerted - entered).total_seconds() / 60))
        except Exception:
            stay_minutes = int(car.get("billable_minutes") or 0)
    ...
    conn.execute(
        """INSERT OR IGNORE INTO alerts(
            ...
            entry_time, event_time, stay_minutes, assigned_spot,
    ...
    try:
        if event_class == "gate_action":
            ...
            if spot_type == "EntrySpot" and direction == "CarIn":
                ...
                # Total stay = when the car first entered the facility until it finally left.
                ...
                try:
                    ...
                    departed = datetime.strptime(server_time, "%Y-%m-%d %H:%M:%S")
                    total_stay_minutes = max(
                        1, math.ceil((departed - entered).total_seconds() / 60)
                    )
                except Exception:
...
.alert{display:grid;grid-template-columns:90px 120px 1fr 80px 80px 80px;gap:6px;align-items:center;padding:8px;margin:6px 0;border-radius:8px;background:#2a1217;border-left:4px solid #ef4444;font-size:11px}
...
<div>
<span class="plate">{{a.plate if a.plate else 'PLATE PENDING'}}</span>
<div class="mini">{{a.event_time or a.created_at}}</div>
</div>
<div class="reason" title="{{a.reason}}">{{a.reason}}</div>
<div class="optional"><b>{{a.stay_minutes or 0}} min</b><div class="mini">stay</div></div>
<div class="optional"><b>{{'%.2f'|format(a.expected_amount or 0)}}</b><div class="mini">expected</div></div>
...
<div class="scroll">
<table>
<tr><th>Plate</th><th>Status</th><th>Spot</th><th>Entered</th><th>Stay</th><th>Charge</th><th>Payment</th><th>Action</th></tr>
{% for c in cars %}
<tr>
<td><b>{{c.plate}}</b></td>
<td>
...
<td>{{c.entry_time[11:16] if c.entry_time else '-'}}</td>
<td>{{c.total_stay_minutes or c.billable_minutes or 0}}m</td>
...
def export_cars():
    ...
    writer.writerow([
        ...
        "total_stay_minutes","billable_minutes",
    ...
    for r in rows:
        writer.writerow([
            ...
            r["total_stay_minutes"], r["billable_minutes"],
