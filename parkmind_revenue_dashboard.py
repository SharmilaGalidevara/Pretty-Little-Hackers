...
def build_hourly_revenue(rows, hours=12):
    """Build recent hourly revenue buckets from accepted payments."""
    now_hour = datetime.now().replace(minute=0, second=0, microsecond=0)
    buckets = []
    lookup = {}

    for offset in range(hours - 1, -1, -1):
        point = now_hour - timedelta(hours=offset)
    ...
    for row in rows:
        # Older DB rows may not have payment_time populated, so use the
        # exit timestamp as a safe visualization fallback.
        ts = (
            row.get("payment_time")
            or row.get("exit_arrival_time")
        ...
        key = str(ts)[:13]
        if key in lookup:
            lookup[key]["revenue"] += float(row.get("actual_paid") or 0)

    peak = max([b["revenue"] for b in buckets] + [0.0])
    for b in buckets:
        b["height"] = round((b["revenue"] / peak) * 100, 1) if peak else 0.0
        ...
        if event_class == "gate_action":
            ...
            if ok:
                upsert_car(
                    ...
                    payment_time=data.get("ServerDateTime") or datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                )
...
<title>PARKMIND — Revenue Analytics</title>
<style>
*{box-sizing:border-box}
body{font-family:'Segoe UI',Arial;margin:0;background:#0b1120;color:#e5e7eb}
header{height:62px;padding:0 18px;background:#111827;border-bottom:1px solid #293548;display:flex;align-items:center;justify-content:space-between}
.brand{font-size:20px;font-weight:800;color:#67e8f9}.sub,.note{font-size:10px;color:#94a3b8}
a{color:#67e8f9;text-decoration:none}.page{padding:14px;max-width:1450px;margin:auto}
.kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:9px;margin-bottom:10px}
.kpi,.card{background:#111827;border:1px solid #263244;border-radius:12px;padding:12px}
.kpi .v{font-size:24px;font-weight:800}.kpi .k{font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px}
.good{color:#86efac}.cyan{color:#67e8f9}.warn{color:#fbbf24}.danger{color:#f87171}
.grid{display:grid;grid-template-columns:1.35fr .85fr;gap:10px}.grid2{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}
h2{font-size:13px;margin:0 0 10px;text-transform:uppercase;color:#cbd5e1;letter-spacing:.7px}
.chart{height:235px;display:flex;align-items:flex-end;gap:6px;padding:12px 4px 28px;border-bottom:1px solid #334155}
.col{flex:1;height:100%;display:flex;align-items:flex-end;justify-content:center;position:relative}
.bar{width:72%;min-width:7px;background:linear-gradient(180deg,#22d3ee,#0284c7);border-radius:5px 5px 1px 1px;position:relative}
.bar:hover:after{content:attr(data-tip);position:absolute;bottom:100%;left:50%;transform:translateX(-50%);background:#020617;color:#fff;padding:5px 7px;border-radius:6px;font-size:10px;white-space:nowrap;z-index:3}
.label{position:absolute;top:calc(100% + 6px);font-size:9px;color:#94a3b8;white-space:nowrap}
.rank{display:grid;grid-template-columns:55px 1fr 72px;gap:8px;align-items:center;margin:9px 0;font-size:11px}
.track{height:9px;background:#1e293b;border-radius:6px;overflow:hidden}.fill{height:100%;background:#38bdf8;border-radius:6px}
table{width:100%;border-collapse:collapse;font-size:11px}th,td{padding:7px;border-bottom:1px solid #263244;text-align:left}th{color:#94a3b8;font-size:9px;text-transform:uppercase}
...
<header>
<div>
<div class="brand">PARKMIND Revenue Analytics</div>
<div class="sub">Owner / Admin business view · accepted payments only · refreshes every 10 seconds</div>
</div>
<div><a href="/">← Operations</a> &nbsp; <a href="/export/cars">Export CSV</a></div>
</header>

<div class="page">
<div class="kpis">
<div class="kpi"><div class="v good">{{'%.2f'|format(total_revenue)}}</div><div class="k">Total collected revenue</div></div>
<div class="kpi"><div class="v cyan">{{'%.2f'|format(today_revenue)}}</div><div class="k">Revenue today</div></div>
<div class="kpi"><div class="v">{{paid_cars}}</div><div class="k">Successful payments</div></div>
<div class="kpi"><div class="v warn">{{'%.2f'|format(avg_ticket)}}</div><div class="k">Average revenue / paid car</div></div>
<div class="kpi"><div class="v danger">{{failed_payments}}</div><div class="k">Payment problems / holds</div></div>
</div>

<div class="grid">
<div class="card">
<h2>Revenue trend — last 12 hours</h2>
{% if has_revenue %}
<div class="chart">
{% for x in hourly %}
<div class="col">
<div class="bar" style="height:{{x.height}}%" data-tip="{{x.label}} · {{'%.2f'|format(x.revenue)}}"></div>
<div class="label">{{x.label}}</div>
</div>
{% endfor %}
</div>
<div class="note">Each bar is accepted revenue recorded in that hour.</div>
{% else %}
<div class="empty">No accepted payments yet.</div>
{% endif %}
</div>
...
<h2>Revenue mix</h2>
{% if revenue_mix %}
{% for x in revenue_mix %}
<div class="rank">
<div>{{x.name}}</div>
<div class="track"><div class="fill" style="width:{{x.percent}}%"></div></div>
<div><b>{{'%.2f'|format(x.revenue)}}</b></div>
</div>
{% endfor %}
{% else %}
<div class="empty">No paid parking/charging data yet.</div>
{% endif %}
<div class="note">Breakdown of recorded parking versus charging charges.</div>
</div>
</div>

<div class="grid2">
<div class="card">
<h2>Top revenue parking spots</h2>
{% if top_spots %}
{% for x in top_spots %}
<div class="rank">
<div><b>{{x.spot}}</b></div>
<div class="track"><div class="fill" style="width:{{x.percent}}%"></div></div>
<div>{{'%.2f'|format(x.revenue)}}</div>
</div>
{% endfor %}
{% else %}
<div class="empty">No paid spot history yet.</div>
{% endif %}
</div>
...
<table>
<tr><th>Time</th><th>Plate</th><th>Spot</th><th>Stay</th><th>Revenue</th></tr>
{% for r in recent_paid %}
<tr>
<td>{{r.display_payment_time}}</td>
<td><b>{{r.plate}}</b></td>
<td>{{r.actual_spot or r.assigned_spot or '-'}}</td>
<td>{{r.display_stay}}</td>
<td class="good"><b>{{'%.2f'|format(r.actual_paid or 0)}}</b></td>
</tr>
{% endfor %}
</table>
{% if not recent_paid %}<div class="empty">No successful payments yet.</div>{% endif %}
</div>
</div>
</div>
</body>
...
{% if is_admin %}<a href="/admin/business">Revenue</a><a href="/export/cars">Export</a>{% endif %}
...
<h2 style="margin:0">Revenue Snapshot</h2>
<a href="/admin/business">Open full revenue analytics →</a>
</div>
<div style="display:grid;grid-template-columns:145px 1fr;gap:14px;align-items:end;margin-top:9px">
<div>
<div style="font-size:25px;font-weight:800;color:#86efac">{{'%.2f'|format(stats.total_revenue)}}</div>
<div class="mini">accepted revenue</div>
</div>
<div style="height:68px;display:flex;align-items:flex-end;gap:4px">
{% for x in revenue_snapshot %}
<div title="{{x.label}} · {{'%.2f'|format(x.revenue)}}" style="flex:1;height:{{x.height}}%;min-height:2px;background:#0ea5e9;border-radius:3px 3px 0 0"></div>
{% endfor %}
</div>
</div>
</div>
...
def dashboard():
    ...
    revenue_rows = [
        dict(r) for r in conn.execute(
            """SELECT plate, actual_paid, payment_time, exit_arrival_time, departure_time
               FROM cars
               WHERE payment_status='PAID'
                 AND COALESCE(actual_paid,0) > 0"""
        ).fetchall()
    ]
    conn.close()
    revenue_snapshot = build_hourly_revenue(revenue_rows, hours=8)
    ...
    paid_rows = [
        dict(r) for r in conn.execute(
            """SELECT *
               ...
               WHERE payment_status='PAID'
                 AND COALESCE(actual_paid,0) > 0
               ORDER BY COALESCE(payment_time, exit_arrival_time, departure_time, '') DESC"""
        ).fetchall()
    ]

    failed_payments = int(conn.execute(
        """SELECT COUNT(*) AS n
           FROM cars
           WHERE status='PAYMENT_HOLD'
              OR payment_status IN ('INVALID','CHARGE_ERROR')"""
    ).fetchone()["n"] or 0)
    conn.close()

    total_revenue = sum(float(r.get("actual_paid") or 0) for r in paid_rows)
    paid_cars = len(paid_rows)
    avg_ticket = total_revenue / paid_cars if paid_cars else 0.0

    today_key = datetime.now().strftime("%Y-%m-%d")
    today_revenue = 0.0
    for r in paid_rows:
        ts = r.get("payment_time") or r.get("exit_arrival_time") or r.get("departure_time")
        if str(ts or "").startswith(today_key):
            today_revenue += float(r.get("actual_paid") or 0)

    hourly = build_hourly_revenue(paid_rows, hours=12)
    has_revenue = any(x["revenue"] > 0 for x in hourly)

    # Spot revenue ranking.
    spot_totals = {}
    parking_total = 0.0
    charging_total = 0.0

    for r in paid_rows:
        spot = r.get("actual_spot") or r.get("assigned_spot") or "Unknown"
        spot_totals[spot] = spot_totals.get(spot, 0.0) + float(r.get("actual_paid") or 0)
        parking_total += float(r.get("parking_cost") or 0)
        charging_total += float(r.get("charging_cost") or 0)

    top_raw = sorted(spot_totals.items(), key=lambda item: item[1], reverse=True)[:8]
    top_peak = max([value for _, value in top_raw] + [0.0])
    top_spots = [
        {
            "spot": spot,
            "revenue": revenue,
            "percent": round(revenue / top_peak * 100, 1) if top_peak else 0.0
        }
        for spot, revenue in top_raw
    ]

    mix_total = parking_total + charging_total
    revenue_mix = []
    if mix_total > 0:
        revenue_mix = [
            {
                "name": "Parking",
                "revenue": parking_total,
                "percent": round(parking_total / mix_total * 100, 1)
            },
            {
                "name": "Charging",
                "revenue": charging_total,
                "percent": round(charging_total / mix_total * 100, 1)
            }
        ]

    recent_paid = paid_rows[:10]
    now_text = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    for r in recent_paid:
        payment_ts = (
            r.get("payment_time")
            or r.get("exit_arrival_time")
            or r.get("departure_time")
        )
        r["display_payment_time"] = str(payment_ts)[11:19] if payment_ts else "-"

        stay_seconds = int(r.get("total_stay_seconds") or 0)
        if not stay_seconds and r.get("entry_time"):
            stay_seconds = seconds_between(
                r.get("entry_time"),
                r.get("departure_time") or now_text
            )
        r["display_stay"] = format_duration(stay_seconds)
...
def export_cars():
    ...
    writer.writerow([
        ...
        "expected_amount","actual_paid","payment_time","payment_status","status",
